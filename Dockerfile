...
WORKDIR /src
COPY ["Umbraco/Umbraco.csproj", "Umbraco/"]
RUN dotnet restore "Umbraco/Umbraco.csproj"
COPY . .
WORKDIR "/src/Umbraco"
RUN dotnet build "Umbraco.csproj" -c $BUILD_CONFIGURATION -o /app/build

FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "Umbraco.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
COPY Umbraco/entrypoint.sh /entrypoint.sh
